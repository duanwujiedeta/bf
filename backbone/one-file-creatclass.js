...
var ReactComponent = {
    ...
    Mixin: {
        ...
        // --------------
        // `construct`
        // `实例化组件时触发`，生成`props`，`props[OWNER]`，`_lifeCycleState`属性
...
// `创建组件时触发`，用来拓展`Constructor`构造函数的原型，继承`spec`。
function mixSpecIntoComponent(Constructor, spec) {
    ...
    for (var name in spec) {
        ...
        if (ReactCompositeComponentMixin.hasOwnProperty(name)) {// ReactCompositeComponentMixin对象上的方法只能重写updateComponent方法
        ...
        if (specPolicy != null) {// ReactCompositeComponentInterface中有的方法，如果被重新定义，则报错
        ...
        if (proto.hasOwnProperty(name)) {// 原型链中，如果已经有mixins componentWillMount componentDidMount componentWillReceiveProps componentWillUpdate componentDidUpdate componentWillUnmount等方法，则报错
        ...
        } else if (proto.hasOwnProperty(name)) {// 同样的属性被多次定义，则生成多个回调函数
...
var ReactCompositeComponentMixin = {
    ...
    // --------------
    // `construct`
    // `实例化组件时触发`，生成`state`，`_pendingState`，`_compositeLifeCycleState`属性
    construct: function(initialProps, children) {
...
var ReactCompositeComponent = {
    ...
    // --------------
    // `createClass`
    // `创建组件时触发`，传入一个对象`spec`，给生成的构造函数的原型链进行拓展
    createClass: function(spec) {

        // --------------
        // `Constructor`
        // `实例化组件时触发`，传入用于渲染`dom`等属性`initialProps`和子元素`children`
    ...
    // --------------
    // `autoBind`
    // `创建组件时触发`，创建组件时，会重新把`unbound.__reactAutoBind`，重新赋值给原型链的`__reactAutoBindMap`属性
