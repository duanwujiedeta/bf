...
// `顶部事件的时候触发`，`cb`是plugin（`SimpleEventPlugin`、`EnterLeaveEventPlugin`等）对应的`executeDispatch`方法，或者`EventPluginUtils_1.executeDispatch`方法
...
// `顶部事件的时候触发`，`executeDispatch`是plugin（`SimpleEventPlugin`、`EnterLeaveEventPlugin`等）对应的`executeDispatch`方法，或者上面的`executeDispatch`方法
...
// `顶部事件的时候触发`，传入`domId`，收集冒泡和捕获的回调函数`_dispatchListeners`和`_dispatchIDs`
...
// `顶部事件的时候触发`，传入`abstractEvents`，并两端收集回调函数和`_dispatchIDs`到`abstractEvents`中
...
// --------------
// `abstractEventQueue`
// `顶部事件的时候触发`，把`abstractEvent`实例传入到该元素中，元素是`abstractEvent`实例
...
// `顶部事件的时候触发`，用于获取注入的`SimpleEventPlugin`、`EnterLeaveEventPlugin`等，传入`abstractEvent`
...
// `顶部事件的时候触发`，传入顶部事件，遍历`plugin`，以便解析出`abstractEvent`对象
...
// `顶部事件的时候触发`，用于合并`abstractEvents`到`abstractEventQueue`中
...
// `顶部事件的时候触发`，把触发的二次加工事件传入；拿到plugin，最后执行`abstractEvent`并释放
...
// `顶部事件的时候触发`，循环执行`executeDispatchesAndRelease`函数，其参数为`abstractEvent`
...
// --------------
// `handleTopLevel`
// `顶部事件的时候触发`，解析出`abstractEvents`对象;
...
// --------------
// `traverseParentPath`
// `顶部事件的时候触发`，根据`start`的`id`，前后拿`id`,`arg`是`abstractEvent`实例
...
var ReactInstanceHandles = {
    ...
    // --------------
    // `traverseTwoPhase`
    // `顶部事件的时候触发`，传入`abstractEvent`，`cb`是`accumulateDirectionalDispatches`方法
...
var SimpleEventPlugin = {
    ...
    // --------------
    // `abstractEventTypes`
    // `顶部事件的时候触发`，AbstractEvent的 key-value对象，主要用于它的type参数
    abstractEventTypes: {
        mouseDown: {
    ...
    // --------------
    // `extractAbstractEvents`
    // `顶部事件的时候触发`，根据顶部触发的事件，生成对应的abstractEvent对象，并前后收集回调函数和`_dispatchIDs`
...
// `顶部事件的时候触发`，根据顶部事件类型进行解析`abstractEvent`的type参数
SimpleEventPlugin.topLevelTypesToAbstract = {
