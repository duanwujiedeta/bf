// --------------
// 在外部生成`ReactDOM`对象，并把`ReactDOM`和`React`传入`factory`中
...
(function (global, factory) {
  ...
  // --------------
  // 提取第二个到最后一个参数成为`args`数组，传入`printWarning`
  ...
  // --------------
  // 使用`console[level]`来输出`args`，`format`是控制台的样式
  ...
  // --------------
  // 记录数据的三个变量`allNativeEvents`、`registrationNameDependencies`、`possibleRegistrationNames`
  ...
  // --------------
  // 注册`registrationName`、和`registrationName+'Capture'`双向事件
  ...
  // --------------
  // 给`registrationNameDependencies`、`registrationNameDependencies`、`allNativeEvents`三个对象添加数据
  ...
  // --------------
  // `canUseDOM`用于判断是否支持浏览器的`DOM`
  var canUseDOM = !!(typeof window !== 'undefined' && typeof window.document !== 'undefined' && typeof window.document.createElement !== 'undefined');


  // --------------
  // 给`propertyInfoRecord`的`type`属性，含有7个属性
  ...
  // --------------
  // `ATTRIBUTE_NAME_START_CHAR`属性标识符的开头字母
  var ATTRIBUTE_NAME_START_CHAR = ":A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD";
  /* eslint-enable max-len */

  // --------------
  // `ATTRIBUTE_NAME_CHAR`属性字符串标识符
  var ATTRIBUTE_NAME_CHAR = ATTRIBUTE_NAME_START_CHAR + "\\-.0-9\\u00B7\\u0300-\\u036F\\u203F-\\u2040";
  // --------------
  // `ROOT_ATTRIBUTE_NAME`根元素属性标识符
  var ROOT_ATTRIBUTE_NAME = 'data-reactroot';
  // --------------
  // `VALID_ATTRIBUTE_NAME_REGEX`属性标识符的正则表达式
  var VALID_ATTRIBUTE_NAME_REGEX = new RegExp('^[' + ATTRIBUTE_NAME_START_CHAR + '][' + ATTRIBUTE_NAME_CHAR + ']*$');
  var hasOwnProperty = Object.prototype.hasOwnProperty;

  // --------------
  // `illegalAttributeNameCache`非法属性的缓存对象、`validatedAttributeNameCache`合法属性对应的缓存对象
  var illegalAttributeNameCache = {};
  var validatedAttributeNameCache = {};
  // --------------
  // 判断`attributeName`标识符是否合法，并将其缓存
  ...
  // --------------
  // 把`propertyInfo.type`值为`RESERVED`(0)和事件属性`on`、`On`、`oN`、`ON`，设置为`忽略属性`
  ...
  // --------------
  // 把`value`类型为`symbol`，或`value`为`boolean`类型且`name`不是开头为`data-`、`aria-`的属性标识符设置为`移除警报属性`
  ...
  // --------------
  // 把`value`为`null`、`value`为`undefined`，`shouldRemoveAttributeWithWarning`（`value`类型为`symbol`，或`value`为`boolean`类型且`name`不是开头为`data-`、`aria-`的属性），或`propertyInfo.type`为`BOOLEAN`且`!value`，或`propertyInfo.type`为`OVERLOADED_BOOLEAN`且`value === false`，或`propertyInfo.type`为`NUMERIC`且`isNaN(value)`，或`propertyInfo.type`为`POSITIVE_NUMERIC`且`isNaN(value) || value < 1`，设置为`移除属性`
  ...
  // --------------
  // `properties`所有的属性都是通过数组递归循环生成，数量是有限的，不含有的则为`null`
  ...
  // --------------
  // 匹配`-a`或`:a`的正则表达式，直接用作转驼峰的方法
  var CAMELIZE = /[\-\:]([a-z])/g;

  // --------------
  // 任何不是驼峰的字符串，满足了条件，都会转为驼峰
  ...
  // --------------
  // 匹配任何`javascript`脚本的正则表达式
  var isJavaScriptProtocol = /^[\u0000-\u001F ]*j[\r\n\t]*a[\r\n\t]*v[\r\n\t]*a[\r\n\t]*s[\r\n\t]*c[\r\n\t]*r[\r\n\t]*i[\r\n\t]*p[\r\n\t]*t[\r\n\t]*\:/i;
  var didWarn = false;

  // --------------
  // 用来检验一个`url`是否含有`js`脚本，有则抛出错误
  ...
  // --------------
  // 获取`node`的所有属性，均要通过`propertyInfo`的性质判断来获取，直到拿到对应值
  ...
  // --------------
  // 所有获取`node`的`attribute`都是通过`node.getAttribute`来获取
  ...
  // --------------
  // 所有设置`node`的`property`都是通过`propertyInfo`来进行设置
...

  {
    ...
    // --------------
    // 所有`describeNativeComponentFrame`执行的存储缓存`WeakMap`对象
    componentFrameCache = new PossiblyWeakMap();
  }

  // --------------
  // 所有函数`fn`，获取它的定位，'\n    at test'，`fn`传入非函数类型则报错
  ...
  // --------------
  // 所有`Component`传入都返回`false`，直到它的`prototype`含有`isReactComponent`属性
  ...
  // --------------
  // 所有`React`组件类型都会得到描述，得不到的则为`""`空描述
  ...
  // --------------
  // 所有`workInProgress`链表的描述，换行返回输出
  ...
  // --------------
  // 给定三个参数，从前到后依次获取`displayName`，取不到就拿最后一个参数替代
  ...
  // --------------
  // 所有`type`传入都获取`displayName`
  function getContextName(type) {
    return type.displayName || 'Context';
  }

  // --------------
  // 所有组件的`type`传入都能获取名字，获取不了则为`null`
  ...
  // --------------
  // 直接调用就获取`current._debugOwner`的组件名
  ...
  // --------------
  // 直接设置`current`、`isRending`、`ReactDebugCurrentFrame.getCurrentStack`
  ...
  // --------------
  // 直接返回`value`的原本值，除非`value`是`function, symbol`，则为空
  ...
  // --------------
  // 当`elem`为`checkbox`或`radio`时，则返回正确
  ...
  // --------------
  // 直接给`node`的`value`属性设置追踪，以一种缓存的形式来进行获取和设置
  ...
  // --------------
  // 直接校验`node`的`value`值是否与`tracker`的相等
  ...
  // --------------
  // 只有含有`checked`、`value`属性的，就会返回`true`
  function isControlled(props) {
    var usesChecked = props.type === 'checkbox' || props.type === 'radio';
    return usesChecked ? props.checked != null : props.value != null;
  }
  // --------------
  // 直接根据`props`生成一个新的对象并返回
  ...
  // --------------
  // 直接给`element`生成`_wrapperState`属性
  ...
  // --------------
  // 直接设置`element`的`checked`、`value`、`defaultChecked`或`defaultValue`
  ...
  // --------------
  // 以布尔值的形式直接设置`element`的`checked`、`value`、`defaultChecked`或`defaultValue`
  ...
  // --------------
  // 直接更新`element`和它的子元素的`checked`、`value`、`defaultChecked`或`defaultValue`
  ...
  // --------------
  // 判断`props`的所有`children`是否是合规的属性
  ...
  // --------------
  // `props`的两个属性`value`、`defaultValue`，必须是`multiple`和数组并存
  ...
  // --------------
  // 直接根据`propValue`来更新`node.options`
  ...
  // --------------
  // 给`element`初始化`_wrapperState`属性
  ...
  // --------------
  // 以`props.value`或`props.defaultValue`来进行更新`element`的`options`
