...
var ReactCompositeComponentMixin = {
    ...
    // --------------
    // `setState`
    // `重新渲染dom时触发`，`partialState`是新传入的`state`对象
    ...
    // --------------
    // `replaceState`
    // `重新渲染dom时触发`，`completeState`是新传入的合并的`state`对象
    ...
    // `重新渲染dom时触发`，用于强制更新`nextProps`是props属性、`nextState`是合并后的`state`
    ...
    // --------------
    // `updateComponent`
    // `重新渲染dom时触发`，执行完 `render` 方法后，进行更新
...
var ReactMultiChildMixin = {
    ...
    // --------------
    // `updateMultiChild`
    // `重新渲染dom时触发`，`nextChildren`是重新生成`id`的`result`对象
...
ReactNativeComponent.Mixin = {
    ...
    // --------------
    // `renderComponent`
    // `重新渲染dom时触发`，`nextProps`被渲染的组件的属性
    receiveProps: function(nextProps, transaction) {
    ...
    // `重新渲染dom时触发`，传入`props`属性，更新元素的属性
    ...
    // --------------
    // `_updateDOMChildren`
    // `重新渲染dom时触发`，`nextProps`是`props`的属性对象
