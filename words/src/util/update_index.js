var list = [{"lid":"lueylid","indexKey":912}];
...
openDB("luey", "indexKey").then((db) => {
    /*getDataAll(db, "indexKey").then((res) => { console.log(JSON.stringify(res)); })*/
    list.forEach((v, k) => {
        updateDB(db, "indexKey", v).then((res) => { console.log("更新成功"); });
    })
