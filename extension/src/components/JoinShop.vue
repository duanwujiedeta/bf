<template>
  ...
  >
    ...
    >
      <el-form-item>
        <p class="start-jz">加入店铺</p>
      </el-form-item>
      <p style="margin-bottom: 15px">
        请输入账号信息，加入店铺{{ formData.shop_name }}。
      </p>
      <el-form-item
        label="店铺名称"
        prop="shop_name"
      ...
      >
        <el-input
          ...
          @focus="focusFn('shop_name')"
          @blur="blurFn('shop_name')"
          @change="
            (val) => {
...
<script>
import { urlToJsonFn } from "@/util/common";
...
export default {
  ...
  methods: {
    getParams({ account_email, account_id }) {
      ...
      this.formData.shop_name = params["shop_name"];
    ...
    async checkRegistEmail() {
      let res = await this.$http.post("shop/check-shop-account-email", {
    ...
    async register() {
      ...
      params.type = type;
      let res = await this.$http.post("shop/account-invite", params);
      if (res.code == 0) {
    ...
    async initToken(token) {
      ...
      if (res.code == 0) {
        ...
        let url =
          ...
          "." +
          process.env.VUE_APP_SHOP_HOST_URL;
